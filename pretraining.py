...
from common import (   get_pdn_medium,
                    ImageFolderWithoutTarget, InfiniteDataloader)
from test import Get_pdn_medium, get_pdn_medium, Get_pdn_mediumv2,get_pdn_small,gget_pdn_small
from mobliePDN import MobileNetV2
...
def get_argparse():
    ...
    parser.add_argument('-o', '--output_folder',
                        default='output/pretraining/c+dw*3v3/')
    return parser.parse_args()


# variables
model_size = 'small'
imagenet_train_path = '/home/zwk/dataset/train'
seed = 42
...
def main():
    ...
    if model_size == 'small':
        ...
        pdn = Get_pdn_mediumv2(out_channels, padding=True)
    else:
        raise Exception()
    ...
    # 定义文件路径
    mean_file_path = './channel_mean.npy'
    std_file_path = './channel_std.npy'

    # 检查文件是否存在
    if os.path.exists(mean_file_path) and os.path.exists(std_file_path):
        # 读取保存的 channel_mean 和 channel_std
        channel_mean = np.load(mean_file_path)
        channel_std = np.load(std_file_path)
        # 将 NumPy 数组转换为 PyTorch 张量并移动到 GPU
        channel_mean = torch.from_numpy(channel_mean).to(device)
        channel_std = torch.from_numpy(channel_std).to(device)
    else:
        # 如果文件不存在，重新计算 channel_mean 和 channel_std
        channel_mean, channel_std = feature_normalization(extractor=extractor, train_loader=train_loader)
        # 将张量从 GPU 移动到 CPU 并转换为 NumPy 数组
        channel_mean = channel_mean.cpu().numpy()
        channel_std = channel_std.cpu().numpy()

        # 保存计算结果
        np.save(mean_file_path, channel_mean)
        np.save(std_file_path, channel_std)
    ...
    for iteration, (image_fe, image_pdn) in zip(tqdm_obj, train_loader):
        ...
        # print("image_pdn", image_pdn.shape)
