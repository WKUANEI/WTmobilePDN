...
from common import get_autoencoder, get_pdn_medium, \
    ImageFolderWithoutTarget, ImageFolderWithPath, InfiniteDataloader
from sklearn.metrics import roc_auc_score
from test import Get_pdn_mediumv2, get_pdn_small
...
def get_argparse():
    ...
    parser.add_argument('-o', '--output_dir', default='output/c+dw*3v3')
    parser.add_argument('-m', '--model_size', default='medium',
                        choices=['small', 'medium'])
    parser.add_argument('-w', '--weights', default='output/pretraining/c+dw*3v3/teacher_small_final_state.pth')
    parser.add_argument('-i', '--imagenet_train_path',
                        default='/home/zwk/dataset/train',
                        help='Set to "none" to disable ImageNet' +
                             'pretraining penalty. Or see README.md to' +
                             'download ImageNet and set to ImageNet path')
    parser.add_argument('-a', '--mvtec_ad_path',
                        default='/home/zwk/dataset/mvtec_anomaly_detection',
...
def main():
    ...
    if config.model_size == 'small':
        ...
        teacher = Get_pdn_mediumv2(out_channels)
        student = Get_pdn_mediumv2(2 * out_channels)
    else:
        raise Exception()
    loaded_model = torch.load(config.weights, map_location='cpu')
    if isinstance(loaded_model, torch.nn.Module):
        # 如果加载的是整个模型，则提取 state_dict
        state_dict = loaded_model.state_dict()
    else:
        # 如果加载的是 state_dict，则直接使用
    ...
    for iteration, (image_st, image_ae), image_penalty in zip(
        ...
        with torch.no_grad():
            ...
            # print("teacher_output_st.shape", teacher_output_st.shape)
